...
new(Len, Name) when ?is_u16(Len) andalso is_binary(Name) andalso Len =:= byte_size(Name) ->
    #vterm_atom_ext{len = Len, name = Name}.

-spec internal_vterm_to_binary(T, Opts) -> binary() when T :: t(), Opts :: term().
internal_vterm_to_binary(#vterm_atom_ext{len = Len, name = Name}, _Opts) when Len =:= byte_size(Name) ->
    <<?ATOM_EXT:8, Len:16, Name/binary>>.

-spec simplify(t()) -> atom().
simplify(#vterm_atom_ext{name = Name}) ->
