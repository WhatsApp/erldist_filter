...
new(Len, Name) when ?is_u8(Len) andalso is_binary(Name) andalso Len =:= byte_size(Name) ->
    #vterm_small_atom_ext{len = Len, name = Name}.

-spec internal_vterm_to_binary(T, Opts) -> binary() when T :: t(), Opts :: term().
internal_vterm_to_binary(#vterm_small_atom_ext{len = Len, name = Name}, _Opts) when Len =:= byte_size(Name) ->
    <<?SMALL_ATOM_EXT:8, Len:8, Name/binary>>.

-spec simplify(T) -> A when T :: t(), A :: atom().
simplify(#vterm_small_atom_ext{name = Name}) ->
