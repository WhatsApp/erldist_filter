...
new(Node, Id, Creation) when ?is_vterm_atom_t(Node) andalso ?is_u32(Id) andalso ?is_creation(Creation) ->
    #vterm_reference_ext{node = Node, id = Id, creation = Creation}.

-spec internal_vterm_to_binary(T, Opts) -> binary() when T :: t(), Opts :: term().
internal_vterm_to_binary(#vterm_reference_ext{node = Node0, id = Id, creation = Creation}, Opts) when
...
->
    ...
    <<?REFERENCE_EXT, Node/binary, Id:32, Creation:8>>.

-spec simplify(t()) -> term().
simplify(VTerm = #vterm_reference_ext{}) ->
