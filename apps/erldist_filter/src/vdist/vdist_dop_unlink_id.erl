...
new(Id, FromPid, ToPid) when ?is_vterm_integer_t(Id) andalso ?is_vterm_pid_t(FromPid) andalso ?is_vterm_pid_t(ToPid) ->
    #vdist_dop_unlink_id{id = Id, from_pid = FromPid, to_pid = ToPid}.

-spec has_payload(T) -> boolean() when T :: t().
has_payload(#vdist_dop_unlink_id{}) ->
    false.

-spec into_control_message_vterm(T) -> vterm:tuple_t() when T :: t().
into_control_message_vterm(#vdist_dop_unlink_id{id = Id, from_pid = FromPid, to_pid = ToPid}) ->
    vterm_small_tuple_ext:new(4, [vterm_small_integer_ext:new(?DOP_UNLINK_ID), Id, FromPid, ToPid]).

-spec sequence_id(T) -> vdist:sequence_id() when T :: t().
sequence_id(#vdist_dop_unlink_id{from_pid = FromPid}) when ?is_vterm_pid_t(FromPid) ->
