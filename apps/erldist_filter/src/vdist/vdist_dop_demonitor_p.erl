...
->
    #vdist_dop_demonitor_p{from_pid = FromPid, to_proc = ToProc, ref = Ref}.

-spec has_payload(T) -> boolean() when T :: t().
has_payload(#vdist_dop_demonitor_p{}) ->
    false.

-spec into_control_message_vterm(T) -> vterm:tuple_t() when T :: t().
into_control_message_vterm(#vdist_dop_demonitor_p{from_pid = FromPid, to_proc = ToProc, ref = Ref}) ->
    vterm_small_tuple_ext:new(4, [vterm_small_integer_ext:new(?DOP_DEMONITOR_P), FromPid, ToProc, Ref]).

-spec sequence_id(T) -> vdist:sequence_id() when T :: t().
sequence_id(#vdist_dop_demonitor_p{from_pid = FromPid}) when ?is_vterm_pid_t(FromPid) ->
