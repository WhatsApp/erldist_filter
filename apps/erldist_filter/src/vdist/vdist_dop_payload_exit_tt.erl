...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(vdist_dop_payload_exit_tt).
...
%% Types
-type t() :: #vdist_dop_payload_exit_tt{}.
...
->
    #vdist_dop_payload_exit_tt{from_pid = FromPid, to_pid = ToPid, trace_token = TraceToken}.

-spec has_payload(T) -> boolean() when T :: t().
has_payload(#vdist_dop_payload_exit_tt{}) ->
    true.

-spec into_control_message_vterm(T) -> vterm:tuple_t() when T :: t().
into_control_message_vterm(#vdist_dop_payload_exit_tt{from_pid = FromPid, to_pid = ToPid, trace_token = TraceToken}) ->
    vterm_small_tuple_ext:new(4, [vterm_small_integer_ext:new(?DOP_PAYLOAD_EXIT_TT), FromPid, ToPid, TraceToken]).

-spec sequence_id(T) -> vdist:sequence_id() when T :: t().
sequence_id(#vdist_dop_payload_exit_tt{from_pid = FromPid}) when ?is_vterm_pid_t(FromPid) ->
