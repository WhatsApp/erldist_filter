...
%%%-----------------------------------------------------------------------------
%%% % @format
-module(erldist_filter_router_sup).
...
child_name(RouterNumber) when ?is_router_number(RouterNumber) ->
    erlang:binary_to_atom(
        <<"erldist_filter_router_", (erlang:integer_to_binary(RouterNumber))/bytes, "_sup">>,
        utf8
    ).
...
child_specs(RouterNumber) when ?is_router_number(RouterNumber) ->
    [
        erldist_filter_handler_sup:child_spec(RouterNumber),
        erldist_filter_router:child_spec(RouterNumber)
    ].
