...
// @generated SignedSource<<4e587851ed29a253f2aa4c2c58e5d0ab>>
/*
...
#define RET_MAP_SIZE (6)
    ...
    keys[k++] = ATOM(otp_name_blocklist);
    vals[v++] = edf_config_get_bool(env, &edf_config_global->otp_name_blocklist);
...
erldist_filter_nif_config_get_1(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    } else if (key_term == ATOM(otp_name_blocklist)) {
        val_term = edf_config_get_bool(env, &edf_config_global->otp_name_blocklist);
...
erldist_filter_nif_config_set_2(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (key_term == ATOM(compact_fragments)) {
        ...
        return edf_config_set_bool(env, &edf_config_global->otp_name_blocklist, val_term);
    } else if (key_term == ATOM(redirect_dist_operations)) {
