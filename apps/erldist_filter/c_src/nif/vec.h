...
typedef struct vec_data_owned_s vec_data_owned_t;
...
vec_buf(const vec_t *vec)
{
    switch (vec->tag) {
    case VEC_TAG_OWNED:
        return (uint8_t *)(vec->data.owned.bin.data);
...
vec_capacity(const vec_t *vec)
{
    ...
    case VEC_TAG_OWNED:
        return (size_t)(vec->data.owned.bin.size);
    case VEC_TAG_REF_BIN:
...
{
    return (vec != NULL && vec->tag == VEC_TAG_OWNED);
}
...
{
    return (vec != NULL && (vec->tag == VEC_TAG_NULL || vec->tag == VEC_TAG_OWNED));
}
...
{
    return (vec != NULL &&
            (vec->tag == VEC_TAG_NULL || (vec->tag == VEC_TAG_OWNED && vec->data.owned.len < vec->data.owned.bin.size)));
}
...
vec_remaining_writable_bytes(const vec_t *vec)
{
    ...
    return (vec->data.owned.bin.size - vec->data.owned.len);
}
